...
class EchelonNode(ApiModel):
    ...
    node_name: str = Field(
        ..., min_length=1, max_length=160, examples=["Central Distribution Center"]
    )
    tier: int = Field(
        ...,
        ge=1,
        le=5,
        description="Echelon tier level (1 = top supplier/central, 2+ = downstream).",
    )
    lead_time: float = Field(
        ..., ge=0, description="Nominal internal lead time in periods.", examples=[3.0]
    )
    holding_cost: float = Field(
        ..., gt=0, description="Unit holding cost per period at this echelon.", examples=[2.0]
    )
    demand_mean: float = Field(
        0.0, ge=0, description="Direct customer demand mean at this node.", examples=[100.0]
    )
    demand_std: float = Field(
        0.0,
        ge=0,
        description="Direct customer demand standard deviation at this node.",
        examples=[20.0],
    )
    parent_node_id: str | None = Field(
        None, description="ID of the upstream supplying node (None if root/central DC)."
    )
...
class MultiEchelonRequest(ApiModel):
    ...
    target_service_level: float = Field(
        0.95, gt=0.5, lt=1.0, description="Target customer cycle service level (e.g. 0.95)."
    )
    ...
    def validate_tree_structure(self):
        ...
        for n in self.nodes:
            if n.parent_node_id and n.parent_node_id not in node_ids:
                raise ValueError(
                    f"parent_node_id '{n.parent_node_id}' for node '{n.node_id}' not found in network nodes"
                )
            if n.parent_node_id == n.node_id:
                raise ValueError(f"node '{n.node_id}' cannot be its own parent")
        ...
        for n in self.nodes:
            ...
            if n.tier <= parent.tier:
                raise ValueError(
                    f"node '{n.node_id}' must have a tier greater than its parent "
                    f"'{parent.node_id}'"
                )
...
class EchelonNodeResult(ApiModel):
    ...
    bullwhip_index: float = Field(
        ...,
        description="Theoretical lead-time amplification estimate, not an observed variance ratio.",
    )


class MultiEchelonResponse(ApiModel):
    """Result of coordinated multi-echelon safety-stock analysis."""
